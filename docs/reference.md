...
| Variable | Default | Purpose |
|---|---|---|
| `use_sandbox` | `true` for `pr`/`branch` on a normal host with the OpenShell SDK; skipped inside an OpenShell sandbox | Runs the target-repo checkout and checklist execution inside an OpenShell sandbox. Nested sandboxing is skipped when `OPENSHELL_SANDBOX` / `OPENSHELL_SANDBOX_ID` is set or `/etc/openshell/auth/sandbox.jwt` exists. `OPENSHELL_ENDPOINT` alone is not containment. `plai review --no-sandbox` skips it. Missing SDK on a normal host fails closed. A copy of the SDK in another venv does not count. |
